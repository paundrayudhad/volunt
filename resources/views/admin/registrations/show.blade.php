<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Detail Pendaftaran') }} — {{ $pendaftaran->user?->name ?? 'Pendaftar' }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <p class="text-sm">Event: <span class="font-medium">{{ $pendaftaran->event?->name ?? '-' }}</span></p>
                    <p class="mt-1 text-sm">Organisasi: <span class="font-medium">{{ $pendaftaran->event?->organization?->name ?? '-' }}</span></p>
