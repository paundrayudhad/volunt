...
State machine assignment: `assigned → confirmed/completed/cancelled`; `confirmed → completed/cancelled`; `reassigned` (hasil reassign: assignment sama pindah shift; kuota lama dilepas, baru diisi; history catat shift lama→baru); `completed/cancelled` terminal. Hanya registration `accepted` yang bisa di-assign. Status `completed` NON-SCOPE Phase 4: tak ada path complete(); completion milik evaluasi pasca-event (Phase 5/6, bersama certificate yang mensyaratkan attendance) — keputusan final review 2026-09-19, bukan defect.

Alur scan: lookup `token_hash` → cek expiry/bekas/revoked → assignment aktif → shift satu event → jendela waktu (`start_at − 30 mnt` s/d `end_at`; di luar → 422) → tulis attendance + log + audit. `late` bila check-in lewat `start_at + 15 mnt`. Check-out wajib setelah check-in; check-in dan check-out memakai token berbeda (sekali pakai per aksi). Replay token sama → 422. Check-out kedua dengan key BERBEDA → 422 'Sudah check-out.' (setelah token valid, tanpa log/audit — keputusan final review 2026-09-19). Token orang/event lain → 404. Urutan guard aktual: replay → token-validity → jendela (urutan aman-oracle; narasi lookup-first di atas disederhanakan).
